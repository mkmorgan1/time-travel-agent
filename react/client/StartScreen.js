...
class StartScreen extends Master {
  ...
  create() {
    /*  PORTAL  */
    state.portal = this.physics.add.sprite(550, 570, 'portal').setScale(1.5);

    /*  PLAYER  */
    state.player = this.add.sprite(550, 500, 'player', 3);
    state.player.angle = -20;

    /*  TITLE  */
    state.gameTitle = this.createText(175, 220, 'TIME TRAVEL AGENT', '100px');
    state.gameTitle.angle = -10;

    /*  START  */
    state.start = this.createText(420, 885, 'CLICK TO START', '70px').setInteractive();
    state.start.angle = -10;

    /*  TUTORIAL  */
    state.tutorial = this.createText(465, 955, 'How To Play', '70px').setInteractive();
    state.tutorial.angle = -10;

    state.credits = this.createText(565, 1115, 'Credits', '40px').setInteractive();
